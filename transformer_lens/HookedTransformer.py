...
class HookedTransformer(HookedRootModule):
    ...
    def load_and_process_state_dict(
        ...
        print("skipping fold_value_biases")
    ...
    def fold_value_biases(self, state_dict: Dict[str, torch.Tensor]):
        ...
        for layer in range(self.cfg.n_layers):
            ...
            b_V: torch.Tensor = state_dict[f"blocks.{layer}.attn.b_V"]
            # [head_index, d_head, d_model]
            W_O: torch.Tensor = state_dict[f"blocks.{layer}.attn.W_O"]
            # [d_model]
            ...
            b_V = b_V if b_V.dtype == torch.int8 else b_V.to(torch.float16)
            W_O = W_O if W_O.dtype == torch.int8 else W_O.to(torch.float16)
            # Need to permute the dimensions of W_O
            W_O = W_O.permute(1, 2, 0)
            # Use standard matrix multiplication instead of einsum
            summed = b_V @ W_O
            # use bits and bytes matmut
            # summed = einsum(
            #     "head_index d_head, head_index d_head d_model -> d_model", b_V, W_O
            # )
            folded_b_O = b_O_original + summed
